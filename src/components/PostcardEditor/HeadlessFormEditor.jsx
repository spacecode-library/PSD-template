...
const HeadlessFormEditor = ({ template }) => {
  ...
  // Detect editable elements from the scene
  useEffect(() => {
  ...
  const handleTextChange = (elementId, value) => {
    ...
    // Update the text in the engine
    if (engine) {
      try {
        engine.block.setString(elementId, 'text/text', value);
      } catch (error) {
  ...
  const handleColorChange = (elementId, color) => {
    ...
    try {
      ...
      engine.block.setColor(elementId, 'text/color', { r, g, b, a: 1 });
    } catch (error) {
  ...
  const handleImageReplace = (elementId, imageUrl) => {
    ...
    try {
      // Clear existing fill
      const currentFill = engine.block.getFill(elementId);
      if (currentFill) {
        engine.block.destroy(currentFill);
      }
      
      // Create new image fill
      const imageFill = engine.block.createFill('image');
      engine.block.setString(imageFill, 'fill/image/imageFileURI', imageUrl);
      engine.block.setFill(elementId, imageFill);
    } catch (error) {
  ...
  const handleExport = async (format = 'application/pdf') => {
    ...
    try {
      ...
      const blob = await engine.block.export(scene, format);
      
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      const extension = format === 'application/pdf' ? 'pdf' : 'png';
      a.download = `postcard-${template?.name || 'design'}-${Date.now()}.${extension}`;
      a.click();
  ...
  if (!isLoaded) {
    return <div className="form-based-editor">Loading...</div>;
  }
    ...
    <div className="form-based-editor">
      ...
      <div className="form-sections">
        ...
        <div className="form-section">
          <h3>Text Content</h3>
          {editableElements.filter(el => el.type === 'text').map(element => (
        ...
        {/* Image Elements */}
        {editableElements.filter(el => el.type === 'image').length > 0 && (
          <div className="form-section">
            <h3>Images</h3>
            {editableElements.filter(el => el.type === 'image').map(element => (
              <div key={element.id} className="form-field">
                <label>{getLabelForRole(element.role)}</label>
...
export default HeadlessFormEditor;